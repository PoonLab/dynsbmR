    ...
    perturbation.rate=0., iter.max=20, fixed.param=FALSE, bipartition=NULL) 
{
  ...
  if (is.null(init.cluster)) {
    ...
    if (is.bipartite) {
      ...
      if (nb.cores==1) {
        ...
        pkm1 <- mclapply(rep(64/nb.cores, nb.cores), function(nstart) {
          kmeans(Yaggr[set1,], Q1, nstart=nstart), mc.cores=nb.cores
        })
        i1 <- sapply(pkm1, function(result) result$tot.withinss)
        
        pkm2 <- mclapply(rep(64/nb.cores, nb.cores), function(nstart) {
          kmeans(Yaggr[set2,], Q2, nstart=nstart), mc.cores=nb.cores
        })
        i2 <- sapply(pkm2, function(result) result$tot.withinss)
        init.cluster <- c(pkm1[[which.min(i1)]]$cluster, 
                          Q1 + pkm2[[which.min(i2)]]$cluster)
      } 
      ## perturbation
      ...
      ## non-bipartite (classical case)
      if (nb.cores==1) {
        km <- kmeans(Yaggr, Q, nstart=64)
        init.cluster <- km$cluster
      } else {
        RNGkind("L'Ecuyer-CMRG")
        pkm <- mclapply(rep(64/nb.cores,nb.cores), function(nstart) {
          kmeans(Yaggr, Q, nstart=nstart) 
          }, mc.cores=nb.cores)
  ...
  result <- list(
    ...
    dynsbm=dynsbmcore(
      n.times, n.nodes, Q, as.vector(Y), present, edge.type, K, 
      init.cluster-1, # -1 for compatibility with C++    
      iter.max, nb.cores, directed, self.loop, fixed.param
    )
  )
  ...
  if(is.bipartite) { 
    ...
    if (sum(bipartite.check) > 0) 
      warning("Model estimation is not coherent with bipartition: group ", 
              "membership is not bipartite\n")
  }
